        ...
        <div id="printMe">
          <div class="print-content">
            ...
            <div class="print-base-info print-flex">
              <div
                class="print-flex-item"
              >{{printType=='out'?'出库单位':'入库单位'}}：{{printData.departName}}</div>
            ...
            <div
              class="print-order-content print-flex"
              v-for="item in printData.list"
              :key="item.xh"
            >
...
import { getIssueFinishList, getPrint } from "@/actions/issue";
export default {
  name: "issue_finish",
  data() {
    return {
              ...
              h(
                "Button",
                {
                  ...
                  on: {
                    click: () => {
                      this.printType = "out";
                      getPrint(params.row.id).then(res => {
                        let { list } = res.data;
                        if (list.length <= 8) {
