...
<template>
  <div>
    ...
    <pagination
      ...
      @on-load="loadData"
    ></pagination>
    <Modal v-model="showVerifyModal" title="初审" @on-cancel="handleCacelModal">
      <Form
        :model="verifyForm"
        ref="verifyForm"
        label-position="right"
...
import { getBidFirstList,verifyBid } from "@/actions/bid";
...
export default {
  ...
  methods: {
    loadData() {
      this.loading = true;
      getBidFirstList(this.filter).then(res => {
        this.loading = false;
